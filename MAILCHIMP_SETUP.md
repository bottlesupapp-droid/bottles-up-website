...
> **Updated architecture note:** VIP signups are now stored directly in Supabase (`vip_emails` table, viewable in the CMS at `/cms/vip-list`) via the `vip-subscribe` Supabase Edge Function, which optionally *also* mirrors each signup to Mailchimp if these credentials are configured. Mailchimp is no longer required for the waitlist to work, and the `MAILCHIMP_*` values below are set as **Supabase Edge Function secrets** (`supabase secrets set MAILCHIMP_API_KEY=...`), not in this repo's `.env` or a local Express server (the old `server.js` has been removed). The account/audience/API-key creation steps below are still accurate.
